...
<div class="container">
			...
			<c:forEach items="${blist}" var="blist">
					...
					<br>
					<%-- <h4>현재페이지: ${pageMaker.cri.page}</h4> --%>
					<div class="col-md-offset-3">
						<ul class="pagination" id="pagination">
							
							<c:if test="${pageMaker.cri.page != pageMaker.startPage}">
								<li><a href="blist${pageMaker.makeSearch(pageMaker.cri.page - 1)}">이전</a></li>
							</c:if>
							...
							<c:if test="${pageMaker.cri.page != pageMaker.endPage}">
								<li><a href="blist${pageMaker.makeSearch(pageMaker.cri.page + 1)}">다음</a></li>
							</c:if>
						</ul>
					</div>
		...
		<option value="tc"<c:out value="${scri.searchType eq 'tc' ? 'selected' : ''}"/>>카페전체</option>
      <option value="t"<c:out value="${scri.searchType eq 't' ? 'selected' : ''}"/>>카페명</option>
      <option value="c"<c:out value="${scri.searchType eq 'c' ? 'selected' : ''}"/>>카페 소개</option>
