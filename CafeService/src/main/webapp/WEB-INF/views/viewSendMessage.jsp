...
div#message_target_name,#message_Head_Title,#message_sendTime{
...
<body onresize="parent.resizeTo(600,600)" onload="parent.resizeTo(600,600)">
	<div id="container">
		<!-- pageing Test -->
		<div id="outter">
			<div id="message_Box_Title">보낸 쪽지함</div>
				<div id="message_Header">
					<div id="message_target_name">받은 사람</div>
					<div id="message_Head_Title">제 목</div>
					<div id="message_sendTime">보낸 시간</div>
				</div>
					...
					<c:forEach var="allMessage" items="${viewAll}" varStatus="status">
						<div id="division_area">
							...
							<!-- 제 목 -->
							<div id="message_Date"><fmt:formatDate value="${allMessage.send}" pattern="yyyy-MM-dd"/></div>
							<!-- 보낸 시간 -->
			...
			<div style="display: block; text-align: center;">		
				<c:if test="${paging.startPage != 1 }">
					<a href="viewSendMessage?nowPage=${paging.startPage - 1 }&cntPerPage=${paging.cntPerPage}">&lt;</a>
				</c:if>
				<c:forEach begin="${paging.startPage }" end="${paging.endPage }" var="p">
					<c:choose>
						...
						<c:when test="${p != paging.nowPage }">
							<a href="viewSendMessage?nowPage=${p }&cntPerPage=${paging.cntPerPage}">${p }</a>
						</c:when>
					</c:choose>
				</c:forEach>
				<c:if test="${paging.endPage != paging.lastPage}">
					<a href="viewSendMessage?nowPage=${paging.endPage+1 }&cntPerPage=${paging.cntPerPage}">&gt;</a>
				</c:if>
			</div>
		</div>
		<!-- pageing Test -->
